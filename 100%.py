dictionary = {"inst":25,"mem":20,"reg":11,"PC":0,"IR":"","clock":1,"hazard":0,"pause":0}
memory,registers,instructions = [],[],[]
memory2,registers2,instructions2 = [],[],[]

pipeline,IBR = ["NIL","NIL","NIL","NIL"],["NIL","NIL","NIL"]
labels = {}

def initialize():
   global memory, registers, instructions,memory2, registers2
   memory = [0 for x in range(dictionary["mem"])]
   registers = [0 for x in range(dictionary["reg"])]
   memory2 = [0 for x in range(dictionary["mem"])]
   registers2 = [0 for x in range(dictionary["reg"])]
...
def hazardControl():
   cmd,dep = pipeline[1],pipeline[2:4]
...
def printPipeline():
   ...
   for i in range(dictionary["reg"]):
      registers2[i] = registers[i]
   for i in range(dictionary["mem"]):
      memory2[i] = memory[i]
