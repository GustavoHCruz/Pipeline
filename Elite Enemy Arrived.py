...
dicionario = {inst: 15,mem: 20,reg: 10,PC: 0,IR: "",clock: 1}
...
def lidarJump():
   ...
   for i in range(dicionario[inst]):
      if instrucoes[i].find(":") != -1:
         ...
         labels = labels.append([aux[0],i])
         decremento += 1
         for i in range(dicionario[inst] - decremento):
            instrucoes[i] = instrucoes[i+1]
         instrucoes = instrucoes.pop()
   dicionario[inst] = dicionario[inst] - decremento
